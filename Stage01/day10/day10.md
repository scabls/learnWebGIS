...
将低级单位根据进率转化为与高级单数组合的形式

> **进率**：将数量从一个单位向另一个单位进行转换时,数值之间的比率 
>
> 1min => 1-数值, min-单位, 1min和60s表示的数量相同
>
> 在对同一数量进行单位名称的改写时，就要用到进率。
>
> 1m可以转换为1000mm, 即m向mm转换的进率是1/1000
>
> 60秒可以转换为1分钟, 即秒向分钟转换的进率是60

##### 流程

**思路一**：先求商，再取整，最后求余

求商：使用基本单位的数值除以基本单位到目标单位的进率，得到目标单位的数值。

取整：目标单位数值的小数部分，表示不足一目标单位的数量，这部分不需要进行单位转换。取整去掉小数部分。

求余：使用目标单位到高级单位的进率对取整结果求余，得到不足以向高级单位进行单位转换的目标单位数值。

注：求解基本单位数值时，因为目标单位和基本单位相同，前两步可省略。求解最高级单位时没有更高级单位，所以不需要求余。

**思路二**：先求余，再求商，最后取整

求余：使用基本单位到比目标单位更高一级单位的进率对基本单位的数值求余，得到不足以向高级单位转换的基本单位数值

求商：求余结果除以基本单位到目标单位的进率，得到目标单位的数值

取整：目标单位数值的小数部分，表示不足一目标单位的数量，这部分不需要进行单位转换。取整去掉小数部分。

注：求解基本单位数值时，因为目标单位和基本单位相同，后两步可省略。求解最高级单位时没有更高级单位，所以不需要求余，直接进行后两步。
...
当采用从小到大的思路计算时，也可以在计算前先减去所有低级单位表示的数量，去掉不足一份当前单位的部分
